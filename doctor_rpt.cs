...
namespace project
{
    public partial class doctor_rpt : Form
    {
        public doctor_rpt()
        {
            InitializeComponent();
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void doctor_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from doctor_master", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "doctor_master";
            comboid.ValueMember = "doctorid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "doctor_master";
            combonm.ValueMember = "doctorname";
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\doctor1.rpt";
                    r1.SelectionFormula = "{doctor_master.doctorid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\doctor1.rpt";
                    r1.SelectionFormula = "{doctor_master.doctorname}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\doctor2.rpt";
                    r1.SelectionFormula = "{doctor_master.doctorid}>0";
                    r1.ReportFileName = st;
                }
