...
namespace project
{
    public partial class boold_donate_rpt : Form
    {
        public boold_donate_rpt()
        {
            InitializeComponent();
        }
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void boold_donate_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from blood_donet", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "blood_donet";
            comboid.ValueMember = "pid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "blood_donet";
            combonm.ValueMember = "pnm";
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\blood_donet1.rpt";
                    r1.SelectionFormula = "{blood_donet.pid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\blood_donet1.rpt";
                    r1.SelectionFormula = "{blood_donet.pnm}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\blood_donet2.rpt";
                    r1.SelectionFormula = "{blood_donet.pid}>0";
                    r1.ReportFileName = st;
                }
