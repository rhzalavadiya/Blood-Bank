...
namespace project
{
    public partial class patient_rpt : Form
    {
        public patient_rpt()
        {
            InitializeComponent();
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void patient_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from patient_master", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "patient_master";
            comboid.ValueMember = "pid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "patient_master";
            combonm.ValueMember = "pname";
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\patient1.rpt";
                    r1.SelectionFormula = "{patient_master.pid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\patient1.rpt";
                    r1.SelectionFormula = "{patient_master.pname}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\patient2.rpt";
                    r1.SelectionFormula = "{patient_master.pid}>0";
                    r1.ReportFileName = st;
                }
