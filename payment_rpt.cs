...
namespace project
{
    public partial class payment_rpt : Form
    {
        public payment_rpt()
        {
            InitializeComponent();
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void payment_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from payment", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "payment";
            comboid.ValueMember = "pid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "payment";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\payment1.rpt";
                    r1.SelectionFormula = "{payment.pid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\payment1.rpt";
                    r1.SelectionFormula = "{payment.pnm}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\payment2.rpt";
                    r1.SelectionFormula = "{payment.pid}>0";
                    r1.ReportFileName = st;
                }
