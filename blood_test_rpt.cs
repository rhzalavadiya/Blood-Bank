...
namespace project
{
    public partial class blood_test_rpt : Form
    {
        public blood_test_rpt()
        {
            InitializeComponent();
        }
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void blood_test_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from blood_test", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "blood_test";
            comboid.ValueMember = "bid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "blood_test";
            combonm.ValueMember = "donorname";
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\blood_test1.rpt";
                    r1.SelectionFormula = "{blood_test.bid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\blood_test1.rpt";
                    r1.SelectionFormula = "{blood_test.donorname}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\blood_test2.rpt";
                    r1.SelectionFormula = "{blood_test.bid}>0";
                    r1.ReportFileName = st;
                }
