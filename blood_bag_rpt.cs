...
namespace project
{
    public partial class blood_bag_rpt : Form
    {
        public blood_bag_rpt()
        {
            InitializeComponent();
        }
        ...
        OleDbCommand cmd;
        OleDbDataAdapter da;
        private void blood_bag_rpt_Load(object sender, EventArgs e)
        {
            label7.Text = DateTime.Now.ToShortDateString();
            label2.Text = DateTime.Now.ToShortTimeString();
            radioButton1.Checked = false;
            da = new OleDbDataAdapter();
            cmd = new OleDbCommand("select * from blood_bag", cn);
            da = new OleDbDataAdapter(cmd);
            dt = new DataTable();
            da.Fill(dt);
            comboid.DataSource = dt;
            comboid.DisplayMember = "blood_bag";
            comboid.ValueMember = "bagid";

            combonm.DataSource = dt;
            combonm.DisplayMember = "blood_bag";
            combonm.ValueMember = "dname";
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true || radioButton2.Checked == true || radioButton3.Checked == true)
            {
                ...
                if (radioButton1.Checked)
                {
                    st = Application.StartupPath + "\\report\\blood_bag1.rpt";
                    r1.SelectionFormula = "{blood_bag.bagid}=" + comboid.Text + "";
                ...
                {
                    st = Application.StartupPath + "\\report\\blood_bag1.rpt";
                    r1.SelectionFormula = "{blood_bag.dname}='" + combonm.Text + "'";
                    r1.ReportFileName = st;
                }
                else
                {
                    st = Application.StartupPath + "\\report\\blood_bag2.rpt";
                    r1.SelectionFormula = "{blood_bag.bagid}>0";
                    r1.ReportFileName = st;
                }
